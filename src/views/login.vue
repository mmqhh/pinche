<template>
  <div id="login-page">
    <div id="login-wrap">
      <h1><img id="logoss" src="./../assets/logo3.png">移快拼车</h1>
      <h2>在线拼车平台</h2>
      <div id="input-section">
        <el-input id="login-username" placeholder="用户名" v-model.trim="userinfo.username"></el-input>
        <el-input id="login-password" type="password" placeholder="密码" v-model.trim="userinfo.password" @keyup.native.enter="onLogin"></el-input>
      </div>
      <div id="btn">
      ...
      return {
        ...
        rules: {
          username: [
            ...
            { pattern: /^[a-zA-Z0-9]{4,20}$/, message: "仅限4-20位英文字母或数字" }
          ],
          realname: [
            {
              type: "string",
      ...
      submitSignUpForm() {
        this.$refs["SignUp"].validate(valid => {
          if (valid) {
            ...
            newSignUpform.check_password = MD5(
              newSignUpform.check_password
            ).toString();
            signUp(newSignUpform)
              .then(res => {
              ...
              .catch(err => {
                if (err === "username_UNIQUE must be unique") {
                  this.$message.error(`用户名已被注册 ${err}`);
                } else {
