<template>
  <div>
    <div id="head">
      <span id="title">我的拼车记录</span>
      <span id="tips">&emsp;&emsp;在此查看我拼过的车</span>
      <hr>
    </div>
    <el-table id="orderTable" height="361px" :data="tableData" v-loading.body="listLoading" element-loading-text="载入中，给我一点时间"
      fit border stripe highlight-current-row style="width: 100%" empty-text="我还没拼过车哦">
    ...
    @refreshList="getMyOrderList(currentPage)"
...
import { mapState } from "vuex";
  ...
  import { getMyOrders } from "./../api/pin";
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        vm.getMyOrderList(vm.currentPage);
      });
    },
    methods: {
      getMyOrderList(page) {
        this.listLoading = true;
        getMyOrders(page, this.uid)
          .then(res => {
            this.listLoading = false;
            let table = res.data.myOrderList;
      ...
      handleCurrentChange(currentPage) {
        ...
        this.getMyOrderList(this.currentPage);
      },
